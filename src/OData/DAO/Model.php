 ...
 * Class Model
 *
 * @property int|string id       это поле класса, которое, как правило, является serial полем
 * @property string constant это также поля класса
 */
abstract class Model
{
    ...
    public function __construct($data = null, int $maxLevels = 2, int $currentLevel = 1)
    {
        ...
        if ($currentLevel <= $maxLevels) {
            ...
            if (!isset(ModelCache::$mapCache[$calledClass])) {

                $map = self::readMap();

                ModelCache::$mapCache[$calledClass]['arrayMap'] = $map->fields();
                ModelCache::$mapCache[$calledClass]['reverseMap'] = array_flip(ModelCache::$mapCache[$calledClass]['arrayMap']);
            }
    ...
    final private static function readMap()
    {
        ...
        try {
            ...
            trigger_error("Модельный класс $calledClass не имеет мапинга полей: {$e->getMessage()}", E_USER_ERROR);
        }
    ...
    final private function setModelData($data, $maxLevels, $currentLevel)
    {
        ...
        if ($data !== null) {
            ...
            $arrayMap = ModelCache::$mapCache[$calledClass]['arrayMap']; //$this->arrayMap;
            $reverseMap = ModelCache::$mapCache[$calledClass]['reverseMap']; //$this->reverseMap;
