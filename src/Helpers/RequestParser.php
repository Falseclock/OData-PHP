...
class RequestParser
{
	...
	/**
	 * During parsing of URL we have to decide how to answer with exception, so requested format should be ready
	 *
	 * @throws ReflectionException
	 * @throws Exception
	 */
	private static function readFormat(): void {
		$parameters = [];
	...
	private static function readUrl(): void {
		$requestUri = new Url($_SERVER['REQUEST_URI']);

		if(mb_strpos($requestUri->path, Configuration::me()->getContextPath()) != 0) {
			throw new Exception("Context path not found");
			// FIXME: own exception        }
		}

		self::$request->url = $requestUri;
	}
}